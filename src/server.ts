...
export function startRestServer(ctx: Context, config: any) {
  ...
  // 处理 /steam-bind/query 接口 - 查询用户绑定的 steamid
  fastify.get('/steam-bind/query', async (request, reply) => {
  ...
  fastify.get('/cs-inv', async (request, reply) => {
    logInfo(ctx, config, 'info', 'src/server.ts', `🌐 📥 REST /cs-inv 收到请求: steamid=${(request.query as any).steamid} refresh=${(request.query as any).refresh} ip=${request.ip}`);
    try {
      ...
      async function fetchFromOfficialApi() {
        ...
        try {
          ...
          logInfo(ctx, config, 'warn', 'src/server.ts', `⚡ 🔌 ❌ 🎮 官方 Steam API 请求失败: ${e.message}`);
        }
        return null;
      }
      ...
      async function fetchFromSteamWebApi() {
        ...
        try {
          ...
          if (status === 402) {
            logInfo(ctx, config, 'warn', 'src/server.ts', '🔌 🌍 ⚠️ 💸 steamwebapi.com 配额已用尽 (402)');
          } else {
            logInfo(ctx, config, 'warn', 'src/server.ts', `⚡ 🔌 ❌ 🌐 steamwebapi.com 请求失败 (${status || e.message})`);
          }
        }
        return null;
      }
      ...
      async function fetchPlayerInfo() {
        ...
        if (preferOfficial) {
          ...
          logInfo(ctx, config, 'info', 'src/server.ts', '🔌 🎮 优先使用官方 Steam API...');
          let result = await fetchFromOfficialApi();
          if (result) return result;

          logInfo(ctx, config, 'info', 'src/server.ts', '⚡ 🔌 ⚠️ 🔀 官方 API 失败，回退到 steamwebapi.com...');
          result = await fetchFromSteamWebApi();
          if (result) return result;
        } else {
          // 优先 steamwebapi.com，官方 API 作为回退
          logInfo(ctx, config, 'info', 'src/server.ts', '🔌 🌍 🌐 优先使用 steamwebapi.com...');
          let result = await fetchFromSteamWebApi();
          if (result) return result;

          logInfo(ctx, config, 'info', 'src/server.ts', '⚡ 🔌 ⚠️ 🔀 steamwebapi.com 失败，回退到官方 Steam API...');
      ...
      // 获取玩家信息
      logInfo(ctx, config, 'info', 'src/server.ts', '🎮 👤 开始获取玩家信息...');
      const playerInfo = await fetchPlayerInfo();
      logInfo(ctx, config, 'info', 'src/server.ts', `✔️ 🎮 ✅ 👤 玩家信息获取成功: ${playerInfo.personaname}`);
      ...
      if (useCache) {
        ...
        if (cached.length) {
          ...
          if (age < config.invDbCacheDays) {
            ...
            usedCache = true;
            logInfo(ctx, config, 'info', 'src/server.ts', `💿 🗄️ 💾 ✅ REST: 使用数据库缓存: ${steamid}`);
          }
        }
      }
      if (!usedCache) {
        logInfo(ctx, config, 'info', 'src/server.ts', '🎒 📦 开始获取库存数据...');
        const invRes = await requestWithRetry(
          () => axiosWithProxy.get(invUrl), { label: 'Steam库存数据', ctx });
        invData = invRes.data;
        logInfo(ctx, config, 'info', 'src/server.ts', `✔️ 🎒 ✅ 📦 库存数据获取成功，物品数量: ${invData.total_inventory_count || 0}`);
      ...
      logInfo(ctx, config, 'info', 'src/server.ts', '📥 📡 ✅ 准备返回响应...');
      return responseData;
    } catch (error) {
      logInfo(ctx, config, 'error', 'src/server.ts', `⚡ 🔌 ❌ 💥 REST API 错误: ${error.stack || error}`);
      const inventoryError = describeInventoryError(error);
      logInfo(ctx, config, 'warn', 'src/server.ts', `🧭 REST /cs-inv 失败说明: ${formatInventoryErrorLog(inventoryError)}`);
      // 确保返回有效的 JSON 响应
      try {
        return reply.status(500).send({
          success: false,
          error: inventoryError.originalMessage || 'Internal server error',
          title: '发生未知错误',
          upstreamStatus: inventoryError.status,
          upstreamStatusText: inventoryError.statusText,
          errorCode: inventoryError.code,
          possibleReason: inventoryError.possibleReason,
          suggestion: inventoryError.suggestion,
        });
      } catch (replyError) {
        logInfo(ctx, config, 'error', 'src/server.ts', `⚡ 📥 ❌ 💥 响应错误: ${replyError.stack || replyError}`);
        // 作为最后的尝试，返回一个简单的错误响应
        return {
  ...
  fastify.get('/cs-inv/image', async (request, reply) => {
    try {
      ...
      logInfo(ctx, config, 'error', 'src/server.ts', `⚡ ❌ 🖼️ REST API 图片代理错误: ${error.stack || error}`);
      return reply.status(500).send({
  ...
  fastify.get('/steam-player/image', async (request, reply) => {
    try {
      ...
      logInfo(ctx, config, 'error', 'src/server.ts', `⚡ 🔗 ❌ 👤 REST API 玩家头像代理错误: ${error.stack || error}`);
      return reply.status(500).send({
        success: false, error: error.message || 'Internal server error', });
    }
  });
  ...
  fastify.listen(
    {
      ...
      if (err) {
        logInfo(ctx, config, 'error', 'src/server.ts', `🚀 ⚡ ❌ 🌐 REST 服务器启动失败: ${err}`);
        return;
      }
      logInfo(ctx, config, 'info', 'src/server.ts', `🚀 ✔️ ✅ 🌐 REST 服务器已启动: ${address}`);
    });
