    ...
    .action(async ({ session }, profLink) => {
      ...
      if (!config.steamWebApiKey) {
        const _r = await replyWithMarkdownKeyboard(
          ...
          config,
          '获取SteamID操作',
          '🔑 未配置 steamWebApiKey，无法使用 getid 功能。请在插件设置中填写来自 steamwebapi.com 的 API Key。',
        );
        if (_r !== undefined) return _r;
      }
      ...
      try {
        ...
        if (status === 402) {
          const _r = await replyWithMarkdownKeyboard(
            ...
            config,
            '获取SteamID操作',
            '💸 steamwebapi.com 配额已用尽 (402)，请稍后再试或充值配额。',
          );
          if (_r !== undefined) return _r;
        }
        ctx.logger.error(
          `[src/commands/getid.ts] [error] ❌ 🔍 请求失败: ${e.message}`,
        );
        const _r = await replyWithMarkdownKeyboard(
          session,
          ctx,
