  ...
  ctx
    ...
    .action(async ({ session }, profLink) => {
      ...
      if (!config.steamWebApiKey) {
        const _r = await replyWithMarkdownKeyboard(
          session, ctx, config, '获取SteamID操作', '🔑 未配置 steamWebApiKey，无法使用 steam-getid 功能。请在插件设置中填写来自 steamwebapi.com 的 API Key。');
        if (_r !== undefined) return _r;
      }
      ...
      try {
        ...
        // 查缓存
        if (config.enableSteamGetIdDbCache) {
          const cached = await ctx.database.get(
            'cs_steam_getid_cache_vincentzyu_fork', { url: profLink });
          if (cached.length) {
            const age = (Date.now() - cached[0].cached_at) / 86400000;
            if (age < config.steamGetIdCacheDays) {
              data = {
                personaname: cached[0].personaName, steamid: cached[0].steamId, };
              logInfo(ctx, config, 'info', 'src/commands/steam-getid.ts', `💿 💾 steam-getid 缓存命中: ${profLink}`);
            }
          }
        }
        ...
        if (!data) {
          ...
          const response = await requestWithRetry(
            () => axiosWithProxy.get(profUrl), { label: 'steam-getid-steamwebapi', ctx });
          data = response.data;

          // 写入缓存
          if (config.enableSteamGetIdDbCache) {
            await ctx.database.upsert('cs_steam_getid_cache_vincentzyu_fork', [
              {
        ...
        logInfo(ctx, config, 'error', 'src/commands/steam-getid.ts', `⚡ ❌ 🔍 请求失败: ${e.message}`);
        const _r = await replyWithMarkdownKeyboard(
